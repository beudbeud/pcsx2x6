...
// frame whose saved registers have since been clobbered). Observed on the ARM64 libretro
// build: GCC popped recExecute's frame before tail-calling EnterRecompiledCode, JIT-era
// calls trashed it, and the first exit-request returned into garbage (the boot-race stall).
