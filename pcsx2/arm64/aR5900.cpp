...
static void recResetRaw()
{
	...
	// mapped into it, whether the emit side uses x20-relative fastmem (recUseBackpatchFastmem),
...
static void recConstApplyNativeEffects(u32 op, RecGprConstState& state)
{
	...
	switch (opcode)
	{
		...
		case 0x22: case 0x26: case 0x1A: case 0x1B: // LWL/LWR/LDL/LDR (write rt)
			recConstSetUnknown(state, rt);
			return;
...
// single Ldr/Str through REFASTMEMBASE (x20); a fault backpatches to the slow path via
...
bool armTryEmitFastmemScalar32(u32 pc, bool is_load, const a64::Register& data)
{
	...
	if (is_load)
		armAsm->Ldr(data.W(), a64::MemOperand(REFASTMEMBASE, RXARG1));
	else
		armAsm->Str(data.W(), a64::MemOperand(REFASTMEMBASE, RXARG1));
	recRecordFastmem(code_start, pc, RXARG1.GetCode(), data.GetCode(), 32, /*sign*/ false, is_load);
	return true;
}

static bool recTryTranslateCachedLoad(u32 bits, bool sign, u32 rt, u32 rs, s32 imm, RecGprCacheState& cache, u32 pc)
{
	...
	if (recUseBackpatchFastmem(pc))
	{
		...
		// (RADDR = zero-extended 32-bit vaddr), so [x20 + vaddr] lands inside the 4 GB window.
		const u8* code_start = armGetCurrentCodePointer();
		switch (bits)
		{
			case 8:  sign ? armAsm->Ldrsb(dst.X(), a64::MemOperand(REFASTMEMBASE, RADDR))
			              : armAsm->Ldrb(dst.W(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
			case 16: sign ? armAsm->Ldrsh(dst.X(), a64::MemOperand(REFASTMEMBASE, RADDR))
			              : armAsm->Ldrh(dst.W(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
			case 32: sign ? armAsm->Ldrsw(dst.X(), a64::MemOperand(REFASTMEMBASE, RADDR))
			              : armAsm->Ldr(dst.W(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
			case 64: armAsm->Ldr(dst.X(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
		}
...
static bool recTryTranslateCachedStore(u32 bits, u32 rt, u32 rs, s32 imm, RecGprCacheState& cache, u32 pc)
{
	static const a64::Register RADDR = a64::x9;
	static const a64::Register RHOST = a64::x10;
	...
	if (recUseBackpatchFastmem(pc))
	{
		...
		switch (bits)
		{
			case 8:  armAsm->Strb(src.W(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
			case 16: armAsm->Strh(src.W(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
			case 32: armAsm->Str(src.W(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
			case 64: armAsm->Str(src.X(), a64::MemOperand(REFASTMEMBASE, RADDR)); break;
		}
...
static bool recTryTranslateCachedLoadQuad(u32 rt, u32 rs, s32 imm, RecGprCacheState& cache, u32 pc)
{
	static const a64::Register RADDR = a64::x9;
	static const a64::Register RHOST = a64::x10;
	...
	if (recUseBackpatchFastmem(pc))
	{
		...
		const u8* code_start = armGetCurrentCodePointer();
		armAsm->Ldr(RQSCRATCH, a64::MemOperand(REFASTMEMBASE, RADDR));
...
static bool recTryTranslateCachedStoreQuad(u32 rt, u32 rs, s32 imm, RecGprCacheState& cache, u32 pc)
{
	static const a64::Register RADDR = a64::x9;
	static const a64::Register RHOST = a64::x10;
	...
	if (recUseBackpatchFastmem(pc))
	{
		...
		const u8* code_start = armGetCurrentCodePointer();
		armAsm->Str(RQSCRATCH, a64::MemOperand(REFASTMEMBASE, RADDR));
		vtlb_AddLoadStoreInfo(reinterpret_cast<uptr>(code_start),
			static_cast<u32>(armGetCurrentCodePointer() - code_start), pc,
			/*gpr*/ 0, /*fpr*/ 0, RADDR.GetCode(), RQSCRATCH.GetCode(),
...
static bool recTryTranslateCachedOp(u32 op, RecGprCacheState& cache, u32 pc)
{
	const u32 opcode = op >> 26;
	const u32 rs = (op >> 21) & 0x1f;
	const u32 rt = (op >> 16) & 0x1f;
	...
	switch (opcode)
	{
		...
		case OP_LB:  return recTryTranslateCachedLoad(8,  true,  rt, rs, imm, cache, pc);
		case OP_LBU: return recTryTranslateCachedLoad(8,  false, rt, rs, imm, cache, pc);
		case OP_LH:  return recTryTranslateCachedLoad(16, true,  rt, rs, imm, cache, pc);
		case OP_LHU: return recTryTranslateCachedLoad(16, false, rt, rs, imm, cache, pc);
		case OP_LW:  return recTryTranslateCachedLoad(32, true,  rt, rs, imm, cache, pc);
		case OP_LWU: return recTryTranslateCachedLoad(32, false, rt, rs, imm, cache, pc);
		case OP_LD:  return recTryTranslateCachedLoad(64, false, rt, rs, imm, cache, pc);
		case OP_LQ:  return recTryTranslateCachedLoadQuad(rt, rs, imm, cache, pc);

		case OP_SB: return recTryTranslateCachedStore(8,  rt, rs, imm, cache, pc);
		case OP_SH: return recTryTranslateCachedStore(16, rt, rs, imm, cache, pc);
		case OP_SW: return recTryTranslateCachedStore(32, rt, rs, imm, cache, pc);
		case OP_SD: return recTryTranslateCachedStore(64, rt, rs, imm, cache, pc);
		case OP_SQ: return recTryTranslateCachedStoreQuad(rt, rs, imm, cache, pc);
...
static bool recTranslateOpOptimized(u32 op, RecGprConstState& const_state, RecGprCacheState& cache, u32 pc)
{
	...
	if (recTryTranslateCachedOp(op, cache, pc))
	{
	...
	switch (opcode)
	{
		...
		case 0x00:
			switch (funct)
			{
				...
				// SYNC — the interpreter handler is empty (memory ordering barrier with
				// no effect on the emulated single-memory model). Native no-op: emit
				// nothing and keep the block intact instead of single-stepping it.
		...
		// Unaligned load/store (ported from ARMSX2) — inline instead of interpreter
		// single-steps (hot in memcpy-style loops).
		case 0x22: armEmitLWL(rt, rs, imm); return true; // LWL
		case 0x26: armEmitLWR(rt, rs, imm); return true; // LWR
		case 0x2A: armEmitSWL(rt, rs, imm); return true; // SWL
		case 0x2E: armEmitSWR(rt, rs, imm); return true; // SWR
		case 0x1A: if (recTryFuseUnalignedLoad(pc, /*is_ldl*/ true, rt, rs, imm)) return true;
			armEmitLDL(rt, rs, imm); return true; // LDL
		case 0x1B: if (recTryFuseUnalignedLoad(pc, /*is_ldl*/ false, rt, rs, imm)) return true;
			armEmitLDR(rt, rs, imm); return true; // LDR
		case 0x2C: if (recTryFuseUnalignedStore(pc, /*is_sdl*/ true, rt, rs, imm)) return true;
			armEmitSDL(rt, rs, imm); return true; // SDL
		case 0x2D: if (recTryFuseUnalignedStore(pc, /*is_sdl*/ false, rt, rs, imm)) return true;
			armEmitSDR(rt, rs, imm); return true; // SDR
		...
		// ~0.5M/interval). When CHECK_CACHE is on, inline-interpret it in-block (it
		// only reads rs, writes no GPR, never touches cpuRegs.pc, raises no exception
		// and triggers no code invalidation) rather than block-terminating + single-
		// stepping — same strategy as the COP0 ops below.
		case 0x2F:
			if (CHECK_CACHE)
...
// --------------------------------------------------------------------------------------
//  Execution-weighted interpreter-fallback profiler (diagnostic)
// --------------------------------------------------------------------------------------
// Counts how often each opcode is *executed* through the interpreter fallback (not how
// many distinct ones are compiled), so testers' logs identify the real EE hot path. The
// EE thread is the SC3 bottleneck (perf log: EE 100%, GS ~75%), and most of that cost is
// the inline interpreter calls this rec still emits for COP2 (VU0 macro) and a few other
...
// interval into `out` (so the caller can route it through the libretro INFO log, which is
// what RetroArch shows — raw stderr is invisible there), then resets so each line reflects
// ~5s of execution. Always writes a line, including "total=0", so a zero result is itself a
// verdict (EE cost is native codegen / memory traffic, not interpreter fallbacks).
	...
	const auto top6 = [](const u64* arr, u32 count, char* dst, size_t cap) {
		...
		for (u32 i = 0; i < count; i++)
		{
			for (u32 j = 0; j < 6; j++)
			{
				if (arr[i] > cnt[j])
				{
					for (u32 k = 5; k > j; k--) { cnt[k] = cnt[k - 1]; idx[k] = idx[k - 1]; }
					cnt[j] = arr[i]; idx[j] = i; break;
				}
			}
		}
		size_t w = 0;
		for (u32 j = 0; j < 6 && cnt[j]; j++)
			w += std::snprintf(dst + w, cap - w, "%s0x%02x=%llu", j ? " " : "", idx[j],
				static_cast<unsigned long long>(cnt[j]));
		if (w == 0)
...
// kill that precedes every fallback (COP0/COP2/LQC2/SQC2 and delay-slot fallbacks) means no
// guest value lives in caller-saved host regs here, so clobbering x0 for the arg is safe;
// RESTATEPTR(x19) is callee-saved across the call.
static void recEmitFallbackCount(u32 op)
{
	armAsm->Mov(RWARG1, op);
	armEmitCall(reinterpret_cast<const void*>(recCountFallback));
}
...
static void recGenDispatchers()
{
	...
	if (CHECK_FASTMEM)
		armLoadPtr(REFASTMEMBASE, &vtlb_private::vtlbdata.fastmem_base); // x20 = host-MMU fastmem base
...
static bool recTryFuseUnalignedLoad(u32 pc, bool is_ldl, u32 rt, u32 rs, s32 imm)
{
	...
	const u8* code_start = armGetCurrentCodePointer();
	armAsm->Ldr(a64::x10, a64::MemOperand(REFASTMEMBASE, a64::x9));
...
static bool recTryFuseUnalignedStore(u32 pc, bool is_sdl, u32 rt, u32 rs, s32 imm)
{
	...
	const u8* code_start = armGetCurrentCodePointer();
	armAsm->Str(a64::x10, a64::MemOperand(REFASTMEMBASE, a64::x9));
...
static void recRecompile(u32 startpc)
{
	...
	{
		recResetRaw();
		// recResetEE() defers an in-flight reset by setting BOTH eeRecNeedsReset and
		// eeRecExitRequested (the latter to bail out to recExecute at the next event
		// test). But we've just performed that reset inline and are about to compile
		// and run the requested block, so the bail-out is redundant. Leaving it set
		// makes recEventTest fastjmp back out to recExecute right after the first
		// post-reset event test — an extra round-trip that intermittently stalls the
		// EE on ARM64 (observed at the arcade boot.elf entry). Clear it: the reset is
		// done, keep executing.
	...
	for (;;)
	{
		...
		// Un-compilable op (syscall / COP0 / MMI SIMD / ...).
		if (compiled == 0)
		{
