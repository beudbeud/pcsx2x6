...
// ------------------------------------------------------------------------
// LWC1: fpr[ft].UL = mem32[GPR[rs] + imm]. Backpatch fastmem when eligible, else the
// slow-path vtlb helper (same as the GPR loads); the 32-bit result is written to the
// FPR's low word. (FPR0 is a real register, so there is no rt==0 discard as for GPRs.)
