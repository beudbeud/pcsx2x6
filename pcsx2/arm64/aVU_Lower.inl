...
void mVU_XGKICK_(u32 addr)
{
	...
	// GetGSPacketSize smuggles the EOP flag in bit 31 on its !REC_VU1 /
	// XGKICKHACK paths; unmasked it makes `size > diff` always true and
	// `size - diff` a ~2GB bogus transfer. This path always sends the whole
	// packet, so the flag itself is irrelevant here.
	size &= 0x7FFFFFFFu;
