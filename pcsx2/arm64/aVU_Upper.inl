...
// bcLane: out-param for the lane-indexed FMUL broadcast fold (yaps2 06d4ff00d4, idea from
// ARMSX2's tryEmitFmulLaneBroadcast — Tyler Bochard, GPLv3). -1 = Ft is a materialized
// full-width operand (the normal path). 0..3 = Ft is the RAW source VF register (read-only
// mapping, broadcast NOT materialized) and the caller's multiply step must fold the lane
// select into a by-element FMUL. Only set when canLaneFold (the sole Ft consumer is the
...
static void setupFtReg(microVU& mVU, a64::VRegister& Ft, a64::VRegister& tempFt, int opCase, int clampType, int& bcLane, bool canLaneFold)
{
	...
	opCase2
	{
		// Lane-indexed FMUL fold: when the only consumer is the multiply step and no Ft clamp
		// will be emitted, skip materializing the broadcast (allocReg + Dup) and let the
		// consumer emit Fmul Vd.4S, Vn.4S, Vm.S[_bc_] — bit-identical per lane. willClamp
		// mirrors opCase1: under !willClamp neither the body's clampType&cFt mVUclamp2 nor
		// SSE_MULPS's clampE-gated clamp emit, so bypassing SSE_MULPS loses no clamps and the
		// raw guest reg is never clamped in place. x86 SSE cannot express this fold.
		const bool willClamp = (clampE || ((clampType & cFt) && !clampE && (CHECK_VU_OVERFLOW(mVU.index) || CHECK_VU_SIGN_OVERFLOW(mVU.index))));
...
static void mVU_FMACa(microVU& mVU, int recPass, int opCase, int opType, bool isACC, microOpcode opEnum, int clampType)
{
	...
	pass2
	{
		...
		// Lane-fold (bcLane>=0) == Dup + SSE_PS[2] under the no-clamp gate; opType==2 (MUL) is
		// the only foldable FMACa op. vm MUST be the scalar .S() view — vixl's by-element Fmul
		// keys element size off vm's format; a V4S vm selects fp16.
