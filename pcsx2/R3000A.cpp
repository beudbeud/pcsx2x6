...
__fi void PSX_INT( IopEventId n, s32 ecycle )
{
	...
	// multiplied by the clock ratio the float result is out of int range. The
	// cast to int is host-defined: x86 CVTTSS2SI returns INT_MIN, ARM64 FCVTZS
	// saturates to INT_MAX — which schedules the next IOP slice ~2^31 EE cycles
	// away and starves the IOP (EE then spins forever on SIF/pad responses).
	// Casting first lets the multiply happen in signed arithmetic where the
	// small-negative case rounds correctly on both hosts.
