...
__fi u32 rcntRcount(int index)
{
	...
	// The arm64 rec's linked blocks defer event tests further than LUT
	// dispatch did, widening the window: a guest lock-free clock (overflow-
	// ISR wrap accumulator + live T_COUNT read) sees time go backwards and
	// can spin forever (NFL 2K5 upstream; same class of loop exists in other
	// titles). Clamp the read to just-before-the-boundary until delivery.
	// The deliverability guard makes this exact: with interrupts blocked
	// (DI/EXL — including inside the handler itself) or the INTC source
	// masked, the guest legitimately observes the wrapped count, as on
	// hardware.
