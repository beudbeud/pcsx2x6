...
void cpuTlbMiss(u32 addr, u32 bd, u32 excode)
{
	...
	// rec path on debug builds, matching the original "don't spam on interp"
	// intent. (vtlb_Miss still logs the first 50 in Release for triage.)
#ifdef __aarch64__
	...
	// exception vector (recEmitInterpTlbMissCheck). We don't use
	// CancelInstruction (longjmp) because that disrupts cycle counting and
	// timing. The x86 recompiler uses CancelInstruction instead.
#ifdef __aarch64__
	if (isRec)
	{
